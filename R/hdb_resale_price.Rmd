...
```{r, time_series}
time_series <- raw[, .(count=.N, min=min(resale_price), max=max(resale_price), median=median(resale_price), mean=mean(resale_price), ci95=1.96*sd(resale_price)/sqrt(.N)), .(town, flat_type, month)]
time_series$min <- time_series$median-time_series$min
time_series$max <- time_series$max-time_series$median
time_series$ci95<- ifelse(time_series$count < 3, 0, time_series$ci95)
...
nrow(street_name)
nrow(address)
write.csv(street_name,'street_name.csv',row.names=FALSE)
write.csv(address,'address.csv',row.names=FALSE)
```